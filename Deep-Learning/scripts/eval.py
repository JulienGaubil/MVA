...
def run(args):
    ...
    if args.idx_sample is not None and len(args.idx_sample):
        ...
        decompositor = Decompositor(trainer)
    ...
    if args.plot_sprites:
        ...
        print(len(trainer.transcribe), trainer.transcribe)
        for i in trainer.transcribe.keys():
            names.append(trainer.transcribe[i])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', "--device", default=0, type=str, help='Cuda ID')
    ...
    parser.add_argument('-o', "--output_path", required=False, default=None, type=str, help='Save Output dir')
