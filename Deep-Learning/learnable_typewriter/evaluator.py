...
class Evaluator(Logger):
    ...
    def __init_metrics__(self):
        ...
        metric_names += [f'prop_clus{i}' for i in range(self.model.n_prototypes)]
        ...
        for (alias, split), metrics in average.items():
            for k, v in metrics.items():
                if k in {'cer', 'wer', 'ser'}:
                    ...
                    self.log(f'[{alias}/{split}] {k}:{v}', eval=True)
    ...
    def er_evaluate_(self, average):
        if self.transcribe is None:
            if self.eval:
                return er_evaluate(self, mapping=self.transcribe_unsupervised, average=average, **self.error_rate_kargs)
            else:
                return er_evaluate(self, average=average, **self.error_rate_kargs)
        else:
            if self.unsupervised_dataset and self.train_end:
                return er_evaluate(self, mapping=self.transcribe, average=average, **self.error_rate_kargs)
            else:
