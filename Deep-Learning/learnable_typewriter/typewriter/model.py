...
class LearnableTypewriter(nn.Module):
    def __init__(self, cfg, transcribe, logger=delta):
        ...
        self.selection = Selection(self.encoder.out_ch, self.sprites.masks_.latent_dim, logger)
        if self.background:
    ...
    def to(self, device):
        ...
        if self.loss.global_loss_weight is not None:
            self.loss.global_loss_weight = self.loss.global_loss_weight.to(device)

        self.loss.__init_beta__(device)
        return self
    ...
    def forward(self, x, return_params=False):
        ...
        y, xr = {}, self.predict_cell_per_cell(x, return_params=return_params)
        loss = self.loss(x, xr)
    ...
    @torch.no_grad()
    def transform(self, x, with_composition=False, pred_labels=[], line_sizes=None, visual_alphabet=False):
        return self.inference(x, with_composition, pred_labels, line_sizes, visual_alphabet)
    ...
    def process_batch_transcriptions(self, y):
        ...
        transcriptions_padded = torch.full(size=(len(y), len_max), fill_value=self.empty_sprite_id//2)  
