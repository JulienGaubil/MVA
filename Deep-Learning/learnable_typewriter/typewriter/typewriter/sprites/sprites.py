...
class Sprites(nn.Module):
    def __init__(self, cfg, logger):
        ...
        samples = init_objects(self.n_sprites, self.color_channels, self.sprite_size, self.proto_init)
        self.prototypes = nn.Parameter(samples)
        self.prototypes.requires_grad = False

        self.masks_ = Generator(self.n_sprites, self.sprite_size, type=cfg['gen_type'], logger=logger)
        self.frozen = False
    ...
    def restart_branch_from(self, i, j, add_noise=False):
        self.masks_.restart_branch_from(i, j, add_noise=add_noise)

    def active_prototypes_set(self):
        return set(np.where(self.active_prototypes.numpy())[0])

    def inactive_prototypes_set(self):
        return set(range(len(self))) - self.active_prototypes_set()
