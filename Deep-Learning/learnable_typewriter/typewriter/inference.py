...
class Inference(object):
    def __init__(self, model):
        self.model = model
        self.dictionary = {i: i for i in range(len(self.model.sprites)+1)}
        self.empty_sprite_id = len(self.model.sprites)

    def get_label(self, i):
        x = self.dictionary.get(i, None)
        if x is None:
            ...
            for p, k in self.sort(y, b, P, true_widths_pos):
                if prev is not None:
                    ...
                    if flags.get('next-symbol', False):
                        pass
                        # seq_label.append((self.empty_sprite_id, flags['distance']))
    ...
    def __call__(self, x, labels=False):
        ...
        y = (self.infer(x, xp) if labels else None)
        return {'x': xp, 'y': y}

    def max_alphabet_size(self):
        return len(self.model.sprites)
