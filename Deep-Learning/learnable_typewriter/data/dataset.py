...
class LineDataset(TorchDataset):
    ...
    def __init__(self, path, height, split, alias, crop_width=None, space=' ', N_min=0, W_max=float('inf'), dataset_size=None, transcribe=None, sep='', supervised=False, padding=None, padding_value=None, filter_by_name=None):
        ...
        if transcribe is None:
            transcribe = dict(enumerate(sorted(self.alphabet)))
            self.transcribe = dict()
            for k,v in enumerate(sorted(self.alphabet)):
                self.transcribe[2*k] = v
                self.transcribe[2*k+1] = v

        self.size = len(self.files)
        self.crop_width = crop_width

        self.matching = {char: list() for num, char in self.transcribe.items()}
        for num, char in self.transcribe.items():
            self.matching[char].append(num)

        self.padding_value = padding_value

        # ## PAD ##
        # if self.padding_value is None and self.padding is not None:
        #     self.padding_f = Pad(self.padding, padding_mode='edge')
        # else:
        ...
        # if self.padding is not None:
            # self.padding_f = Pad(self.padding, fill=self.padding_value, padding_mode='constant')
    ...
    def get_label(self, i):
        if self.annotation is None or self.cropped:
            ...
            return [self.matching.get(char,[-1])[0]//2 for char in transcription]
