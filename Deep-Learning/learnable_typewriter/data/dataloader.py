...
def collate_fn_pad_to_max(batch, supervised=None, alias=None, pad_value=None, max_w=0):
    ...
    xs = torch.cat([pad_right(x, max_w) for x in xs], dim=0)
...
class SequentialAdaptiveDataLoader(object):
    def __init__(self, dataset, batch_size, supervised, num_workers, min_width=0, mean_width=1024, drop_last=True, shuffle=True):
        ...
        print('batch_size @ num_workers, mp.cpu_count()', batch_size, num_workers, mp.cpu_count())
    ...
    def make_out(self,):
        return {'x': torch.cat([pad_right(x, self.cache['max_w']) for x in self.cache['xs']], dim=0),
                'y': self.cache['ys'],
                'w': self.cache['ws'],
