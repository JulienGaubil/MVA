...
class Decompositor(object):
    ...
    def __init__(self, trainer, decompose=True):
        ...
        self.colors = sns.color_palette(cc.glasbey, len(self.model.sprites))
...
###TODO REFACTOR ___ PATH SELECT ONLY###
class DecompositorSupervised(Decompositor):
    def __init__(self,trainer, decompose=True):
        super().__init__(trainer, decompose)
    ...
    def __call__(self, x):
        ...
        with torch.no_grad():
            out = self.model.predict_cell_per_cell(x)
            proto = self.model.prototypes.data.clone()
            self.model.prototypes.data.copy_(self.expand_colors(proto))

            tsf_layers = out['tsf_layers']
            selection = out['selection']
            tsf_layers_colored = self.expand_colors_layers(tsf_layers,selection)
            r = (x['x'], out['tsf_bkgs'], tsf_layers_colored, out['tsf_masks'])
            out['segmentation'] = self.model.compositor(*r)['cur_img']
